...
typedef enum _Emile_Colorspace
{
  ...
  /* The number in between are reserved to preserve compatibility with evas and eet */
...
} Emile_Image_Load_Error; /**< Evas image load error codes one can get - see emile_load_error_str() too. */
...
const char *emile_load_error_str(Emile_Image *source, Emile_Load_Error error);
